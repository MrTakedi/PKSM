...
const firstBoxRecords = seedRecords.map((record, index) => ({ ...record, box: 1, slot: index + 1, gender: genderForIndex[index] }));
...
const initialRecords: PokemonRecord[] = allSlots.map((slot) => firstBoxRecords.find((record) => record.slot === slot.slot && record.box === slot.box) || slot);

const legalMoves = new Set(["Thunderbolt", "Quick Attack", "Electro Ball", "Protect", "Psychic", "Moonblast", "Surf", "Ice Beam", "Dragon Claw", "Aura Sphere", "Shadow Ball", "Toxic"]);

type LegalityIssue = { field: string; message: string; severity: "warning" | "error" };

function getLegalityIssues(record: PokemonRecord): LegalityIssue[] {
  ...
  moves.forEach((move) => {
    if (!legalMoves.has(move)) issues.push({ field: "moves", message: `${move} is not in the current browser legality catalog.`, severity: "warning" });
  });
...
export default function Home() {
  ...
  const [records, setRecords] = useState<PokemonRecord[]>(() => {
    try {
      const stored = JSON.parse(localStorage.getItem("pksm-browser-box") || "null");
      return Array.isArray(stored) && stored.length ? stored : initialRecords;
  ...
  const legalityIssues = draft ? getLegalityIssues(draft) : [];
  ...
  function autoFixDraft() {
    ...
    (draft.moves || []).forEach((move) => {
      ...
      if (!legalMoves.has(move)) { fixedMoves.push(""); clearedInvalid += 1; return; }
