...
import { loadCatalogs } from "@/lib/catalogLoader";
...
export default function Home() {
  ...
  const selectedItemEntry = catalogsReady ? catalogEntryForName(activeCatalog.items, selected?.heldItem) : undefined;
  const draftItemEntry = catalogsReady ? catalogEntryForName(activeCatalog.items, draft?.heldItem) : undefined;
  const legalityIssues = draft ? getLegalityIssues(draft, activeCatalog.generation, catalogsReady) : [];
  ...
  useEffect(() => {
    ...
    void loadCatalogs().then(() => setCatalogsReady(true)).catch((error) => {
