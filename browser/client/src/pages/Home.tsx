...
import { catalogEntryForName, catalogForGeneration, type GameDefinition } from "@/lib/gameData";
import { loadCatalogs, loadOfflineCatalogs } from "@/lib/catalogLoader";
...
function getLegalityIssues(record: PokemonRecord, generation = "VIII", catalogsReady = true): LegalityIssue[] {
...
function CatalogSkeleton({ compact = false }: { compact?: boolean }) {
  return <span className={`catalog-skeleton ${compact ? "compact" : ""}`} role="status" aria-live="polite" aria-label="Loading move and item catalogs"><span className="catalog-skeleton-label">HYDRATING CATALOG</span><span className="catalog-skeleton-bar" /><span className="catalog-skeleton-bar short" /></span>;
}

function ItemDescriptionSkeleton() {
  return <span className="item-description-skeleton" role="status" aria-label="Loading held-item description"><span /><span className="short" /></span>;
}

function ItemCategorySkeleton() {
  return <span className="item-category-skeleton" role="status" aria-label="Loading held-item category"><span /></span>;
}

function CatalogRetryState({ onRetry, compact = false }: { onRetry: () => void; compact?: boolean }) {
  return <span className={`catalog-retry ${compact ? "compact" : ""}`} role="alert"><span>Catalog unavailable</span><button type="button" onClick={onRetry}><RefreshCw size={12} /> Retry</button></span>;
}
...
export default function Home() {
  ...
  const catalogsUsable = catalogsReady || catalogOffline;
  const selectedItemEntry = catalogsUsable ? catalogEntryForName(activeCatalog.items, selected?.heldItem) : undefined;
  const draftItemEntry = catalogsUsable ? catalogEntryForName(activeCatalog.items, draft?.heldItem) : undefined;
  const legalityIssues = draft ? getLegalityIssues(draft, activeCatalog.generation, catalogsUsable) : [];
  const hasLegalityErrors = legalityIssues.some((issue) => issue.severity === "error");

  useEffect(() => {
    if (catalogsReady || (!dummyDataEnabled && !saveFile && !showPokedex && editingId === null)) return;
    ...
    void loadCatalogs().then(() => {
      ...
      setCatalogError(error instanceof Error ? error.message : "The catalog chunk could not be loaded.");
    }).finally(() => setCatalogsLoading(false));
  }, [catalogRetryToken, catalogsReady, editingId, saveFile, showPokedex]);
